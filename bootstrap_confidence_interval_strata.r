calculate.confidence.intervals.strata = function (species) {
#-----------------------------------------------------------------
p = setup.parameters()
loadfunctions()
data.yrs = 1983:2017
...
#oldClass(j) <- c("data.frame", "strata.data")
j$Strata = j$strat
...
#sc.t = Prepare.strata.data(sc)
#sc.t$yr = year(sc.t$sdate)
#----------------------------------------------------

#sw = Stratify_m(j,strat1,k$totwgt_sd)
