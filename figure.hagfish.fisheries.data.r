...
setwd(p$shpdir)
write.shapefile(l, 'hagfish.fisheries.data', p=p)
l.n = l

j = ddply(l,. (yr), summarize, effort = length(traps_retrieved[!is.na(traps_retrieved)]), no.effort = length(traps_retrieved[is.na(traps_retrieved)]))
j$per = round((j$effort/ (j$effort + j$no.effort)) * 100 , 0)
j

unique(l.n$zones)
l.n = subset(l.n, is.na(zones)|zones == 'Mid-Shore' |zones == 'Gully'| zones == 'Slope'| 
               zones == 'NE Channel' | zones == 'Jordan' | zones == 'Georges Basin')

l.n$traps_retrieved = as.numeric(l.n$traps_retrieved)

if(DS %in% c("landings", "complete")) {
    l.r = l[which(is.finite(l$landings)),]
    l.r = l[which(!is.na(l$zone)), ]
