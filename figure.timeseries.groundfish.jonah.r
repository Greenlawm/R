fisheries.timeseries.groundfish.jonah = function(DS, data.yrs = data.yrs) {
#browser()
  p = setup.parameters()
  ...
  j = survey.process(k, species, data.yrs = data.yrs)
  j = j[which(j$yr != 2004), ]
  j = j[which(j$yr != 2018), ]
  ...
  g.inshore = g.yr[which(g.yr$lfa_nafo != '4X-40' & g.yr$lfa_nafo != '4Xn-41' & g.yr$lfa_nafo != '4Xs-41' 
                         & g.yr$lfa_nafo != 35 & g.yr$lfa_nafo !=36 & g.yr$lfa_nafo != 37 & g.yr$lfa_nafo !='5Z-41'),]
  
  if(DS %in% c("offshore", "complete")) {
...
#----------------------------------------------------------
#For each LFA
lfa_area = read.table("C:\\ss\\data\\data\\strata_lfa_area.csv", header=T, sep="," )
...
if(DS %in% c("dwao.lfa.38", "complete")) {

  j.s = jas[which(jas$lfa_nafo == "38"),]
  j.avg = avg[which(avg$lfa_nafo == "38"), 2 ]
...
if(DS %in% c("dwao.lfa.34", "complete")) {
  
  j.s = jas[which(jas$lfa_nafo == "34"),]
  j.avg = avg[which(avg$lfa_nafo == "34"), 2 ]
...
if(DS %in% c("dwao.lfa.33", "complete")) {
  
  j.s = jas[which(jas$lfa_nafo == "33"),]
  j.avg = avg[which(avg$lfa_nafo == "33"), 2 ]
