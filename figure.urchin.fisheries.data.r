figure.urchin.fisheries.data = function(DS) {
#browser()

p = setup.parameters()
l = urchin.logbook.db(DS = "effort")
l$m.id <- seq.int(nrow(l))

#setwd(p$shpdir)
#write.shapefile(l, 'urchin.fisheries.data', p=p)
#l.n = l

j = ddply(l,. (yr), summarize, position = length(longitude[!is.na(longitude)]), no.position = length(longitude[is.na(longitude)]))
j$per = round((j$position/ (j$position + j$no.position)) * 100 , 0)
j

l$yr = l$f.year

l = l[which(l$zone_type != "LFA 38 Zone 1 Dive"),]
l = l[which(l$zone_type != "LFA 38 Zone 2 Dive"),]
l = l[which(l$zone_type != "SWNB Dive"),]
l = l[which(l$yr != 2018),]


if(DS %in% c("landings", "complete")) {
    l.r = l[which(is.finite(l$landings)),]
    l.r = l.r[which(!is.na(l.r$zone_type)), ]
    l.yr = l.r[, c("f.year", "landings", "zone_type")]
    l.yr$landings = l.yr$landings/1000 #convert kgs to mt
    l.yr <- ddply (l.yr, .(f.year, zone_type), summarize, landings = sum(landings))
    ...
    outfile = paste("urchin.yearly.landings")
    fn = file.path( p$mapdir, paste(outfile,"png",sep="." ) )
    
    p1 <- ggplot(l.yr, aes(y=landings, x=yr, group=zone_type)) + theme_economist() + 
      geom_line(aes(linetype = zone_type, color = zone_type), size = 0.75) +
      geom_point(aes(shape=zone_type, color = zone_type), size = 1.5) +
      scale_linetype_manual(values = c("twodash", "dotted", "twodash", "dotted", "twodash","dotted", "dashed", "twodash","solid")) + 
      scale_shape_manual(values = c(1,17,15,16,15,1,15,16,15,16)) +
      scale_color_manual(values = c("red", "red", "blue", "darkblue", "darkgreen", "darkgreen", "black")) +
