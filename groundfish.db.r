  ...
  groundfish.db = function(  DS="complete", p=NULL, taxa="all", datayrs=NULL  ) {
    ...
    if (DS %in% c("redo") ) {
      ...
      # ODBC data dump of groundfish tables
      groundfish.db( DS="gscat.redo", datayrs=datayrs )
      groundfish.db( DS="gsdet.redo", datayrs=datayrs )
      groundfish.db( DS= "gsinf.redo", datayrs=datayrs )
      groundfish.db(DS = "set.base.redo")
      groundfish.db( DS="cat.base.redo" )
      groundfish.db( DS ="det.base.redo")
      #groundfish.db( DS="set.base.redo")
 ...
 # ----------------------
    ...
    if (DS %in% c("gscat", "gscat.redo"  ) ) {
      ...
      ##### #move most of this into survey.process.r script ########
      ...
      set = merge(gscat, gsinf, all.x = T)
      
      #two ways of standardizing the data
      #-----------------------------------------------
      #cftow is based on a standarized tow of 1.75 nm
      set$totwgt_sd = set$totwgt * set$cftow # this is based on a standardized tow of 1.75 nm
      set$totno_sd = set$totno * set$cftow
      
      #sakm2 is based on density per km^2
      set$totwgt_d = set$totwgt / set$sakm2 #this is tot weight per km^2
      set$totno_d = set$totno /  set$sakm2
     
      gscat = set[, c("id", "id2", "mission", "setno", "sdate", "spec", 
                      "totwgt", "totno", "cftow", "sampwgt", "totwgt_sd", "totno_sd", "totwgt_d", "totno_d")] # kg, no/set
  ...
  # ----------------------
    ...
    if (DS %in% c("gsinf", "gsinf.redo" ) ) {
      ...
      gsinf = gsinf[which(gsinf$settype ==1),]
 ...
 # ----------------------
    if (DS %in% c("gsstratum", "gsstratum.odbc.redo")){
      fstratum = file.path( loc,"gsstratum.rdata")
    ...
    odbcClose(connect)
    names(gsstratum) =  tolower( names(gsstratum) )
    save(gsstratum, file=fstratum, compress=T)
    print(fstratum)
    return(fstratum)
 ...
 # ----------------------
    ...
    if (DS %in% c("set.base", "set.base.redo") ) {
      ...
      set = set[, c("id", "sdate", "yr", "strat", "dist_km", "dist_pos", 
