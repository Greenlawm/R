#Function to map proportional symbols over groundfish survey strata that represent their geometric mean catch from xxxx-xxxx
    ...
    #Take the geometric mean of weight and number by year, and then over years
	  yr.m <- ddply (j,. (yr, spec, strat), summarize, lat = mean (lat), lon = mean (lon), 
	                  wgt = gm.mean (totwgt_sd, na.rm=T, zero.propagate = FALSE), tno = gm.mean(totno_sd, na.rm=T, zero.propagate = FALSE))
	  
	  tot.m <- ddply (j,. (spec, strat), summarize, lat = mean (lat), lon = mean (lon), 
	                  wgt = gm.mean (totwgt_sd, na.rm=T, zero.propagate = FALSE), tno = gm.mean(totno_sd, na.rm=T, zero.propagate = FALSE))
	  ...
	  intersect.sp.groundfish (x = j, y=y)
	  ...
	  strata <- merge (strata.df, tot.s, all = T)
	  # Order so shape file works correctly
	  strata <- arrange (strata, order)
	  strata<-strata[!is.na(strata$wgtg),]
	...
	#	for (v in variables) {
		  ...
		  n = j[, c("yr", "lon", "lat", v)]
		 	...
		 	for (i in 1:length(sort(unique(n$yr)))) {
				...
				labname = paste("Geometric Mean kg/tow")
				

		    splot <- ggplot () +
		    geom_polygon (data = strata, aes (x = long, y = lat, group = group, fill = wgtg), colour = "white", size = 0.4) +
		    ...
		    scale_y_continuous ("Latitude", expand = c (0,0), limits = c (39.8, 46.5)) + 
		    scale_x_continuous ( "Longitude", expand = c (0,0), limits = c (-70.5, -56.5)) +
