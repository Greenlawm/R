...
#Cell size can also be adjusted to change the default cell size that is mapped, default = 6km
	...
	raster.map.variables.seacucumber.offshore = function(p=p, grid.fun, variables, cell=NULL) {
	    #browser()
	  ...
	  K = K[which(K$zone == '4X'|K$zone == '4W'|K$zone == '4Vs'),]
    ...
    contours <- load.shapefile(DS = 'contours')
	  ...
	  writeOGR(sfd, ".", "SeaCucumberFisheriesDataUpdate", driver="ESRI Shapefile", overwrite=T)
		...
		ncols <- length(p$extsc.offshore[2]:p$extsc.offshore[1])/cell
		nrows <- (length(p$extsc.offshore[4]:p$extsc.offshore[3])-1)/cell

		#Set rows and columns for the raster stack. Larger cell size because of smaller plots
		ncols.big <- length(p$extsc.offshore[2]:p$extsc.offshore[1])/cell.big
		nrows.big <- (length(p$extsc.offshore[2]:p$extsc.offshore[1])-1)/cell.big
	...
	#	print(v)
		  v= "landings"
			...
			grid <- raster(nrows=nrows, ncols=ncols, ext=p$extsc.offshore, crs=p$geog.proj)
			grid[]<- 1:ncell(grid)
			grid.big <- raster(nrows=nrows.big, ncols=ncols.big, ext=p$extsc.offshore, crs=p$geog.proj)
			...
			for (i in 1:length(sort(unique(M$f.year)))) {
				...
				#Create Rasters
				ras.name <- paste("sc.offshore", v, name, ".tif", sep="")
...
#					z <- z*10
		#		}
			  ...
			  #MG check to see if quantiles are being calculated properly for effort, there doesn't seem to be a lot of red on the maps
				...
				#Plot the variable
				setwd(p$mapdir)
			  fig.name <- paste("sc.offshore", v, name, ".pdf", sep="")
			...
			stack.name<- paste("sc.offshore", v, "stack", ".pdf", sep="")
