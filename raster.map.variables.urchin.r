...
#Cell size can also be adjusted to change the default cell size that is mapped, default = 6km
	...
	raster.map.variables.urchin = function(p=p, DS, cell=NULL) {
	  #browser()
	  ...
	  fd <- K
	  to_extract = c("licence_id", "mon_doc_id", "rnd_weight_kgs" , "landed_date", "community_code", "mon_doc_id", 
	                 "community_code", "date.landed", "month", "landings", "yr", "lat", "lon", "avg.diver.hrs", "avg.diver.mins", 
	                 "tow.dur.hrs", "tow.dur.mins", "no.tows", "no.divers", "avg.time.tow", "depth",  "comments", "date.fished.2", 
	                 "no.tows.2", "uom.default.ft", "fishing.area.no", "bottom.temp" )
	  fd = fd[, to_extract]
	  ...
	  setwd(p$shpdir)
	  writeOGR(sfd, ".", "UrchinFisheriesDataUpdate", driver="ESRI Shapefile", overwrite=T)
		...
		if (DS %in% c("landings.yearly")) {
		  ...
		  for (i in 1:length(sort(unique(M$yr)))) {
		    ...
		    fig.name <- paste(v, name,"urchin", ".png", sep="")
		  ...
		  stack.name<- paste(v, "stack","urchin", ".png", sep="")
		  ...
		  print(levelplot(rstack[[min.plot:max.plot]], at=quant, colorkey=ckey, col.regions=p$seis, alpha.regions=1,
		        ...
		        + layer(sp.polygons(coast, fill='lightgrey', col = NA, under = T)) 
		        + layer(sp.lines(contours, col='dimgrey', alpha=0.6, lwd= 0.4, under = T))
		        )
		...
		if (DS %in% c("landings.allyears")) {
		  ...
		  #Create Rasters
		  ras.name <- paste("urchin", "landings.allyears", ".tif", sep="")
		  ...
		  fig.name <- paste("urchin", "landings.allyears", ".png", sep="")
